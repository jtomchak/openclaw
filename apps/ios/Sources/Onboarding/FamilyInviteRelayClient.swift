...
final class FamilyInviteRelayClient: @unchecked Sendable {
    ...
    static let audience = "openclaw-family-relay"
    ...
    private struct RedeemResponse: Decodable {
        ...
        var resolvedInviteId: String? {
            self.inviteId ?? self.invite?.inviteId
        }
    }
        ...
        key: any FamilyInviteSigningKey) async throws -> FamilyInviteEnrollmentResult
    {
        ...
        let redeem: RedeemResponse = try await self.post(
            ...
            key: key,
            bearerToken: nil)
        guard let inviteId = redeem.resolvedInviteId,
              !inviteId.isEmpty,
        ...
        let edge = try await self.edgeToken(inviteId: inviteId, baseURL: invite.relayBaseURL, key: key)
        ...
        bearerToken: String?) async throws -> Response
    {
        ...
        let canonical = "openclaw-family-proof-v1\nPOST\n\(path)\n\(Self.audience)\n\(timestamp)\n\(nonce)\n\(bodyHash)"
