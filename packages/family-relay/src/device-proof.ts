...
export async function verifyDeviceProof(params: {
  ...
  const publicKeyThumbprint = await p256JwkThumbprint(jwk);
  if (claimedThumbprint !== publicKeyThumbprint) throw new Error("device thumbprint mismatch");
  ...
  return { nonce, publicKeyThumbprint, timestampMs };
}
