...
export async function admitRelayRequest(params: {
  env: { FAMILY_RELAY_GUARD: DurableObjectNamespace };
  ip: string;
  limit: number;
  nowMs: number;
  replayExpiresAtMs: number;
  ...
  for (const partition of [`ip:${params.ip}`, `device:${params.thumbprint}`]) {
    const id = params.env.FAMILY_RELAY_GUARD.idFromName(partition);
    const response = await params.env.FAMILY_RELAY_GUARD.get(id).fetch("https://guard/consume", {
