...
import { createAgentGatewayRpc, type AgentGatewayRpc } from "./gateway-rpc.js";
import { admitRelayRequest, AgentRelayGuard, RelayAdmissionError } from "./relay-guard.js";
import type { RelayEnv } from "./types.js";

export { AgentRelayGuard };
...
async function handleRedeem(
  ...
  if (
    ...
    token.length < 16 ||
    token.length > 4096 ||
    body.deviceThumbprint !== proof.enrollmentKeyThumbprint ||
  ...
  await admitRelayRequest({
    ...
    thumbprint: proof.enrollmentKeyThumbprint,
  });
  const result = await rpc.redeem({
    token,
    enrollmentKeyThumbprint: proof.enrollmentKeyThumbprint,
    publicUrl: publicRelayOrigin(env).origin,
  });
...
async function handleEdgeToken(
  ...
  const status = await rpc.status(invitationId);
  if (
    (status.invitation.state !== "redeeming" && status.invitation.state !== "active") ||
    status.invitation.enrollmentKeyThumbprint !== proof.enrollmentKeyThumbprint
  )
    return errorResponse(403);
...
async function handleGateway(
  ...
  if (claims.cnf.jkt !== proof.enrollmentKeyThumbprint) return errorResponse(403);
  await admitRelayRequest({
    env,
    ip: clientIp(request),
    limit: 60,
...
function invitePage(): Response {
  return new Response(
    "<!doctype html><meta charset=utf-8><title>OpenClaw Agent Invitation</title><p>Open this link in the OpenClaw app.</p>",
    {
      headers: {
...
export async function handleAgentRelayRequest(
  ...
  try {
    ...
    console.error("agent relay request failed", error instanceof Error ? error.message : "unknown");
    return errorResponse(400);
  }
}

export default { fetch: handleAgentRelayRequest } satisfies ExportedHandler<RelayEnv>;
